...
"""
In MMSC, for adding new datasets, dataset builder for datasets need to be
...
to add dataset builder to ease setup of MMSC.

``load`` is used to load a dataset from specific path. ``load`` needs to return
an instance of subclass of ``mmsc.datasets.base_dataset.BaseDataset``.
...
from mmsc.utils.build import build_dataloader_and_sampler
...
class BaseDatasetBuilder(pl.LightningDataModule):
    ...
    def build_dataset(self, config, dataset_type="train", *args, **kwargs):
        """
        Similar to load function, used by MMSC to build a dataset for first
    ...
    def load_dataset(self, config, dataset_type="train", *args, **kwargs):
        """Main load function use by MMSC. This will internally call ``load``
