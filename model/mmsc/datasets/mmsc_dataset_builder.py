...
import mmsc.utils.download as download
import torch
...
from mmsc.datasets.subset_dataset import MMSCSubset
from mmsc.utils.configuration import get_global_config, get_mmsc_env, get_zoo_config
from mmsc.utils.general import get_absolute_path
from omegaconf import open_dict


class MMSCDatasetBuilder(BaseDatasetBuilder):
    ...
    def _download_requirement(
        ...
        download_path = os.path.join(
            get_mmsc_env("data_dir"), "datasets", dataset_name, dataset_variation
        )
    ...
    def _split_dataset_from_train(self, dataset, dataset_type):
        if dataset_type in self.config.split_train.keys() or dataset_type == "train":
            ...
            dataset = MMSCSubset(dataset, indices)
            print(
