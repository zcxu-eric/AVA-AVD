...
from mmsc.utils.download import download_pretrained_model
from mmsc.utils.fileio import PathManager
from mmsc.utils.general import get_current_device, updir
...
def _hack_imports():
    ...
    sys.modules["pythia"] = importlib.import_module("mmsc")
    sys.modules["pythia.utils.configuration"] = importlib.import_module(
        "mmsc.utils.configuration"
    )
...
def _load_pretrained_model(model_name_or_path, *args, **kwargs):
    ...
    assert (
        len(ckpts) == 1
    ), "None or multiple checkpoints files. mmsc doesn't know what to do."
...
class Checkpoint:
    ...
    def save(self, update, iteration=None, update_best=False):
        ...
        model = self.trainer.model
        data_parallel = registry.get("data_parallel") or registry.get("distributed")
        best_der_info = registry.get("best_der_info", None)
