...
from mmsc.utils.general import get_absolute_path
...
class DownloadableFile:
    ...
    Originally taken from ParlAI, this file has been modified for mmsc specific
    use cases.
    ...
    def _parse_url(self, url):
        if url.find(self.mmsc_PREFIX) == -1:
            return url
        else:
            return self.mmsc_PREFIX_REPLACEMENT + url[len(self.mmsc_PREFIX) :]
...
def download_pretrained_model(model_name, *args, **kwargs):
    import omegaconf
    from mmsc.utils.configuration import get_mmsc_env, load_yaml
    from omegaconf import OmegaConf

    model_zoo = load_yaml(get_mmsc_env(key="model_zoo"))
    OmegaConf.set_struct(model_zoo, True)
    OmegaConf.set_readonly(model_zoo, True)

    data_dir = get_absolute_path(get_mmsc_env("data_dir"))
    ...
    try:
        ...
        print(f"No such model name {model_name} defined in mmsc zoo")
        raise e
...
def get_image_from_url(url):
    ...
    img = np.ndarray(Image.open(io.BytesIO(response.content)))
    return img
