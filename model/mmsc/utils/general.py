...
from mmsc.utils.distributed import get_rank, get_world_size
...
def get_mmsc_root():
    ...
    mmsc_root = registry.get("mmsc_root", no_warning=True)
    if mmsc_root is None:
        mmsc_root = os.path.dirname(os.path.abspath(__file__))
        mmsc_root = os.path.abspath(os.path.join(mmsc_root, ".."))
        registry.register("mmsc_root", mmsc_root)
    return mmsc_root
...
def get_absolute_path(paths):
    ...
    if isinstance(paths, str):
        ...
        from mmsc.utils.configuration import get_mmsc_env

        mmsc_root = get_mmsc_root()
        user_dir = get_mmsc_env(key="user_dir")
        if user_dir:
            possible_paths.append(os.path.join(user_dir, paths))
            # check in relative to mmsc relative user dir
            possible_paths.append(os.path.join(mmsc_root, "..", user_dir, paths))

        # Relative to root folder of mmsc install
        possible_paths.append(os.path.join(mmsc_root, "..", paths))
        # Relative to mmsc root
        possible_paths.append(os.path.join(mmsc_root, paths))
...
def extract_loss(report: Dict[str, Any], loss_divisor: int) -> torch.Tensor:
    ...
    # Since losses are batch averaged in MMSC, this makes sure the
    # scaling is right.
...
def get_temporal_bboxes(tbox, size):
    bs = size[0]
    H, W = size[-2:]
    bboxes = torch.zeros((bs, 5), dtype=torch.float32)
    bboxes[:, 0] = torch.arange(bs)
    bboxes[:, 4] = H
    tbox = tbox * W
    bboxes[:, 1] = tbox[:, 0]
    bboxes[:, 3] = tbox[:, 1]
...
def visualize(path, mode='train'):
    if mode == 'train':
        legends = ['audioface', 'sync', 'veri']
        pattern = (rf'.*?{mode}/voxceleb/amsfaceaudio: (.*?),'
                #    rf'.*?{mode}/voxceleb/amsface: (.*?),'
                   rf'.*?{mode}/voxceleb/synchronization: (.*?),'
                   rf'.*?{mode}/voxceleb/verification: (.*?),.*?')
    else:
        legends = ['sync', 'veri']
        pattern = (rf'.*?{mode}/voxceleb/synchronization: (.*?),'
                   rf'.*?{mode}/voxceleb/verification: (.*?),.*?')
    ...
    losses = []
    for line in lines:
        loss = pattern.search(line)
        if loss is not None:
            loss = list(map(float, loss.groups()))
            losses.append(loss)
    for i, legend in enumerate(legends):
        plt.plot(np.arange(len(losses)), np.array(losses)[:,i], label=legend)
    plt.legend()
    plt.show()
# visualize('/home/zcxu/AVD/avdiarization/nohup.out', mode='train')