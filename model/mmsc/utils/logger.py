...
from mmsc.utils.distributed import get_rank, is_master
from mmsc.utils.fileio import PathManager
from mmsc.utils.timer import Timer
...
def setup_output_folder(folder_only: bool = False):
    ...
    """
    save_dir = get_mmsc_env(key="save_dir")
    ...
    env_log_dir = get_mmsc_env(key="log_dir")
    if env_log_dir:
...
def setup_logger(
    ...
    **kwargs,
    ...
    """
    Initialize the MMSC logger and set its verbosity level to "INFO".
    ...
    Args:
        ...
        name (str): the root module name of this logger. Defaults to "mmsc".
    ...
    # Remove existing handlers to add MMSC specific handlers
    if clear_handlers:
...
def setup_very_basic_config(color=True):
    ...
    # log a message even before logging is setup by MMSC.
...
def log_class_usage(component_type, klass):
    """This function is used to log the usage of different MMSC components."""
    identifier = "MMSC"
...
# ColorfulFormatter is adopted from Detectron2 and adapted for MMSC
