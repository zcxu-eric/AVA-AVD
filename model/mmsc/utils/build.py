...
from mmsc.datasets.iteration_strategies import (
    ConstantIterationStrategy,
    IterationStrategy,
    SizeProportionalIterationStrategy,
)
from mmsc.datasets.processors.processors import Processor
from mmsc.utils.configuration import Configuration, get_global_config
from mmsc.utils.distributed import is_dist_initialized, is_master, synchronize
from mmsc.utils.general import get_optimizer_parameters
...
def build_model(
    config: Union[DictConfig, "mmsc.models.base_model.BaseModel.Config"]
) -> "mmsc.models.base_model.BaseModel":
    from mmsc.models.base_model import BaseModel
...
def build_dataset(
    ...
    from mmsc.utils.configuration import load_yaml_with_defaults
...
def build_dataloader_and_sampler(
    ...
    """
    from mmsc.common.batch_collator import BatchCollator, FewShotBatchCollator
    predict = 'avd' in dataset_instance.dataset_name or dataset_instance.dataset_type in ['val', 'test']
    ...
    other_args = {
        ...
        "shuffle": datamodule_config.get(
            "shuffle", training_config.get("shuffle", None)
        ) if not predict else False,
        "batch_size": get_batch_size() if not predict \
                      else 320,
    }

    dataset_config = get_global_config('dataset_config').get(dataset_instance.dataset_name)
    sampler_config = dataset_config.get('sampler', None)
    ...
    # to the codebase
    if not isinstance(dataset_instance, torch.utils.data.IterableDataset) and dataset_instance.dataset_type == 'train':
        other_args = _add_extra_args_for_dataloader(dataset_instance, other_args, sampler_config)
    else:
        other_args.pop("shuffle")

    loader = torch.utils.data.DataLoader(
        dataset=dataset_instance,
        collate_fn=FewShotBatchCollator(
...
def _add_extra_args_for_dataloader(
    ...
    sampler_instace = registry.get_sampler_class(sampler_type)
    
    if sampler_instace is not None:
        # In distributed mode, we use DistributedSampler from PyTorch
        other_args["sampler"] = sampler_instace(
            dataset_instance, sampler_config, 
            batch_size=other_args['batch_size'], shuffle=other_args["shuffle"]
        )
...
def build_classifier_layer(config, *args, **kwargs):
    from mmsc.modules.layers import ClassifierLayer
...
def build_image_encoder(config, direct_features=False, **kwargs):
    ...
    from mmsc.modules.encoders import ImageEncoderFactory, ImageFeatureEncoderFactory
...
def build_encoder(config: Union[DictConfig, "mmsc.modules.encoders.Encoder.Config"]):
    from mmsc.modules.encoders import Encoder
