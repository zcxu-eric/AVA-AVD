...
def der(ref_turns, sys_turns, collar=0.0, ignore_overlaps=False, uem=None):
    ...
    with np.errstate(invalid='ignore', divide='ignore'):
        ...
        ms = miss_speaker_times / scored_speaker_times
        fa = fa_speaker_times / scored_speaker_times
        es = error_speaker_times / scored_speaker_times
    ...
    ms[np.isnan(ms)] = 0 # Numerator and denominator both 0.
    ms[np.isinf(ms)] = 1 # Numerator > 0, but denominator = 0.
    ms *= 100. # Convert to percent.

    fa[np.isnan(fa)] = 0 # Numerator and denominator both 0.
    fa[np.isinf(fa)] = 1 # Numerator > 0, but denominator = 0.
    fa *= 100. # Convert to percent.

    es[np.isnan(es)] = 0 # Numerator and denominator both 0.
    es[np.isinf(es)] = 1 # Numerator > 0, but denominator = 0.
    es *= 100. # Convert to percent.
    ...
    file_to_ms_base = dict(zip(file_ids, ms))
    file_to_ms = {}
    for file_id in uem:
        try:
            ms = file_to_ms_base[file_id]
        except KeyError:
            # Check for any system turns for that file, which should be FAs,
            # assuming that the turns have been cropped to the UEM scoring
            # regions.
            n_sys_turns = len(
                [turn for turn in sys_turns if turn.file_id == file_id])
            ms = 100. if n_sys_turns else 0.0
        file_to_ms[file_id] = ms
    global_ms = file_to_ms_base['ALL']

    file_to_fa_base = dict(zip(file_ids, fa))
    file_to_fa = {}
    for file_id in uem:
        try:
    ...
    file_to_es_base = dict(zip(file_ids, es))
    file_to_es = {}
    for file_id in uem:
        try:
            es = file_to_es_base[file_id]
        except KeyError:
            # Check for any system turns for that file, which should be ess,
            # assuming that the turns have been cropped to the UEM scoring
            # regions.
            n_sys_turns = len(
                [turn for turn in sys_turns if turn.file_id == file_id])
            es = 100. if n_sys_turns else 0.0
        file_to_es[file_id] = es
    global_es = file_to_es_base['ALL']

    return file_to_der, global_der, file_to_ms, global_ms, file_to_fa, global_fa, file_to_es, global_es
