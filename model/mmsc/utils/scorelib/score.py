...
class Scores(namedtuple(
        'Scores',
        ['file_id', 'der', 'ms', 'fa', 'es', 'jer', 'bcubed_precision', 'bcubed_recall',
...
def score(ref_turns, sys_turns, uem, step=0.010, nats=False, jer_min_ref_dur=0.0,
    ...
    file_to_der, global_der, file_to_ms, global_ms, file_to_fa, global_fa, file_to_es, global_es = metrics.der(
    ...
    def compute_metrics(fid, cm, der, ms, fa, es, jer):
        ...
        return Scores(
            fid, der, ms, fa, es, jer, bcubed_precision, bcubed_recall, bcubed_f1,
    ...
    for file_id, cm in iteritems(file_to_cm):
        file_scores.append(compute_metrics(
            file_id, cm, file_to_der[file_id], file_to_ms[file_id], file_to_fa[file_id], file_to_es[file_id], jer=file_to_jer[file_id]))
    global_scores = compute_metrics(
        '*** OVERALL ***', global_cm, global_der, global_ms, global_fa, global_es, global_jer)
