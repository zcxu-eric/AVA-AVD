...
from mmsc.common.sample import convert_batch_to_sample_list
...
from mmsc.utils.general import ckpt_name_from_core_args, foldername_from_config_override
...
class TestReporter(Dataset):
    ...
    def __init__(
        ...
        # self.datamodules = {k: v for k, v in datamodules.items() if 'avd' in k}
        self.datamodules = {k: v for k, v in datamodules.items() if v.val_dataset is not None}
        ...
        self.report_folder_arg = get_mmsc_env(key="report_dir")
        ...
        self.save_dir = get_mmsc_env(key="save_dir")
    ...
    def reshape_and_gather(self, report, key):
        if key in report and torch.is_tensor(report[key]):
